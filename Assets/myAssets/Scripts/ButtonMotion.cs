...
public class ButtonMotion : MonoBehaviour
{
    ...
    public void updateButton(Vector3 controllerPos)
    {
        if (!lockX) transform.position = transform.position + new Vector3 (controllerPos.x, 0, 0);
        if (!lockY) transform.position = transform.position + new Vector3(0, controllerPos.y, 0);
        if (!lockZ) transform.position = transform.position + new Vector3(0, 0, controllerPos.z);
    }

    public void activateButton()
    {
        pillDispenser.GetComponent<PillDispenser>().DispensePill();
    }
